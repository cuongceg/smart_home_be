...
class DeviceController {
      ...
      res.json({
        ...
        message:
          req.user.role === "admin"
            ...
            : "My devices retrieved successfully",
        data: result,
      });
    } catch (error) {
  ...
  static async getDeviceStats(req, res) {
    try {
      let roomStats, typeStats;
      if (req.user.role === "admin") {
        [roomStats, typeStats] = await Promise.all([
          Device.getGlobalStatsByRoom(),
          Device.getGlobalStatsByType(),
        ]);
      } else {
        [roomStats, typeStats] = await Promise.all([
  ...
  static async shareDevices(req, res) {
    try {
      ...
      const result = await Device.shareDevices(
        req.user.id,
        targetUserId,
        deviceIds
      );
  ...
  static async revokeDeviceAccess(req, res) {
    try {
      ...
      const result = await Device.revokeDeviceAccess(
        deviceId,
        req.user.id,
        userId
      );
  ...
  static async getSharedDevices(req, res) {
    try {
      ...
      const sharedDevices = devices.filter(
        (device) => device.access_type === "member"
      );
